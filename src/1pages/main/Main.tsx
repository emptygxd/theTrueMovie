...
const Main = () => {
  ...
  const {
    ...
    queryFn: () => {
      return http.get(
        `/movie?page=1&isSeries=true&limit=50&selectFields=id&selectFields=name&selectFields=alternativeName&selectFields=movieLength&selectFields=poster&selectFields=rating&selectFields=year&selectFields=genres&notNullFields=top250&sortField=top250&sortType=1&lists=series-top250`
      );
    },
    select: (data) => data.data.docs,
    refetchOnWindowFocus: false,
